...
function GPACalculator() {
  ...
  // NEW: State for existing semesters data
  ...
  // Fetch current CGPA and existing semesters on component mount
  useEffect(() => {
    if (user) {
      fetchCurrentCGPA();
  ...
  // NEW: Fetch existing semesters for accurate projection calculation
  const fetchExistingSemesters = async () => {
    try {
      setExistingDataLoading(true);
      const response = await apiService.getAllSemesters();

      if (response.success && response.data) {
        const semesters = response.data;
        setExistingSemesters(semesters);

        // Calculate total unique credits from existing semesters
        const uniqueCredits = calculateUniqueCredits(semesters);
        setTotalExistingCredits(uniqueCredits);
      }
    } catch (err) {
      console.error('Failed to fetch existing semesters:', err);
    } finally {
      setExistingDataLoading(false);
    }
  };

  // NEW: Helper function to calculate unique credits (not counting retakes)
  ...
  // Sync function for graduation planner - NON-BLOCKING
  ...
  // SINGLE handleSubmit function with NON-BLOCKING sync
  const handleSubmit = async (semesterData) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Refresh existing semesters data
        await fetchExistingSemesters();

        // ✅ NON-BLOCKING SYNC WITH GRADUATION PLANNER
        // This won't block GPA saving even if graduation sync fails
        syncWithGraduationPlanner().catch(err => {
          console.warn('⚠️ Graduation sync failed (non-blocking):', err);
      ...
      <div className="row">
        <div className="col-lg-8">
          <GPAForm
            ...
            existingCredits={totalExistingCredits} // NEW: Pass actual existing credits
            existingSemestersCount={existingSemesters.length} // NEW: Pass actual semester count
            existingDataLoading={existingDataLoading} // NEW: Pass loading state
          />
        </div>
        ...
        <div className="col-lg-4">
          <div className="info-card card h-100">
            <div className="card-body">
              ...
              {/* NEW: Existing Semesters Info */}
              {!existingDataLoading && (
                <div className="existing-data-info mt-4 p-3 border rounded">
                  ...
                  <div className="d-flex justify-content-between">
                    ...
                    <strong>{existingSemesters.length}</strong>
                  </div>
                  <div className="d-flex justify-content-between">
                    <span>Unique Credits:</span>
                    <strong>{totalExistingCredits}</strong>
                  </div>
                  {existingSemesters.length > 0 && (
                    <small className="text-muted d-block mt-2">
                      <i className="bi bi-info-circle me-1"></i>
                      Projected CGPA will be calculated based on your actual data.
                    </small>
                  )}
                </div>
              )}
              ...
              {previewData && (
                <div className="quick-preview mt-4 p-3 border rounded">
                  ...
                  <div className="d-flex justify-content-between">
                    <span>Courses:</span>
                    <strong>{previewData.courses || 0}</strong>
                  </div>
                </div>
              )}
            </div>
