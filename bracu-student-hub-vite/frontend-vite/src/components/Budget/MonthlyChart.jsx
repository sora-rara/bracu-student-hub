// src/components/Budget/MonthlyChart.jsx
import React, { useState, useEffect } from 'react';

function MonthlyChart({ transactions, monthlyIncome = 0 }) {
  ...
  useEffect(() => {
    generateCalendar();
    // Calculate stats after calendar is generated
    calculateMonthStats();
  }, [transactions, selectedMonth, selectedYear, monthlyIncome]);
  ...
  // Calculate month statistics - FIXED VERSION
  const calculateMonthStats = () => {
    ...
    // Use the monthlyIncome prop (from Budget Summary user input)
    const monthlyIncomeValue = parseFloat(monthlyIncome) || 0;
    
    // Calculate monthly expense (sum of all expense transactions in the month)
    const totalExpense = monthTransactions
      .filter(t => t.type === 'expense')
      .reduce((sum, t) => sum + parseFloat(t.amount || 0), 0);
    
    // Calculate net balance for the month using user's monthly income from Budget Summary
    const netBalance = monthlyIncomeValue - totalExpense;
    
    // FIXED: Calculate days with transactions in this month
    // We need to count unique days that have any transactions
    const uniqueDaysWithTransactions = new Set();
    
    monthTransactions.forEach(transaction => {
      const dateStr = normalizeDate(transaction.date);
      if (dateStr) {
        uniqueDaysWithTransactions.add(dateStr);
      }
    });
    
    const daysWithTransactions = uniqueDaysWithTransactions.size;
    
    // FIXED: Calculate average daily expense
    // Only count days that actually have expenses (not total days in month)
    ...
    if (expenseDays.size > 0) {
      averageDailyExpense = totalExpense / expenseDays.size;
    }
    ...
    console.log('📊 Fixed Month Stats:', {
      ...
      averageDailyExpense,
      uniqueDaysWithTransactions: Array.from(uniqueDaysWithTransactions)
    });
  };
...
Income: ${formatCurrency(dayData.income)}
Expense: ${formatCurrency(dayData.expense)}
Net: ${formatCurrency(dayData.income - dayData.expense)}`;
  };
  ...
  useEffect(() => {
    if (transactions.length > 0) {
      ...
      console.log('📊 Fixed Month Stats:', monthStats);
      console.log('📋 Monthly Income from Budget Summary:', monthlyIncome);
    }
  }, [transactions, selectedMonth, selectedYear, monthStats, monthlyIncome]);
                    ...
                    <>
                      ...
                      {dayData.hasTransactions && (
                        <div className="day-transactions">
                          {dayData.income > 0 && (
                            <div className="day-income" title={`Income: ${formatCurrency(dayData.income)}`}>
                              +{formatAmount(dayData.income)}
                            </div>
                          )}
                          {dayData.expense > 0 && (
                            <div className="day-expense" title={`Expense: ${formatCurrency(dayData.expense)}`}>
          ...
          <div className="stat-row">
            <div className="stat-item">
              ...
              <div className="stat-value text-success">
                {formatCurrency(monthStats.monthlyIncome)}
              </div>
            </div>
            <div className="stat-item">
            ...
            <div className="stat-item">
              ...
              <div className={`stat-value ${monthStats.netBalance >= 0 ? 'text-success' : 'text-danger'}`}>
                {formatCurrency(monthStats.netBalance)}
              </div>
            </div>
          </div>
          <div className="stat-row">
            <div className="stat-item">
              ...
              <div className="stat-value">
                {monthStats.daysWithTransactions} {monthStats.daysWithTransactions === 1 ? 'day' : 'days'}
              </div>
            </div>
            <div className="stat-item">
              <div className="stat-label">Average Daily Expense</div>
              <div className="stat-value">
                {formatCurrency(monthStats.averageDailyExpense)}
