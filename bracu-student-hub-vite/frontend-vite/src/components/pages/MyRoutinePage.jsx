...
export default function MyRoutinePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setConnect(cData || []);
        setMy(picks); // Could be null

        // Only redirect if picks is explicitly null (not loaded yet)
        if (picks === null) {
          // No picks saved yet, stay on page but show empty state
          // Don't auto-navigate, let user choose
        } else if (picks && (!picks.picks || picks.picks.length === 0)) {
          // Has picks object but empty array, navigate to setup
  ...
  const computed = useMemo(() => {
    // Handle null or empty picks
    if (!my || !my.picks || my.picks.length === 0) {
      return { blocksByDay: {}, clashPairs: [] };
    }
    const idx = new Map();
  ...
  if (my === undefined) return <div style={{ padding: 20 }}>Loading…</div>;

  if (my === null) {
    return (
      <div style={{ padding: 20, textAlign: "center", maxWidth: 600, margin: "0 auto" }}>
        <h2 style={{ marginBottom: 16 }}>Welcome to My Routine</h2>
        <p style={{ marginBottom: 24, fontSize: 16, color: "#666" }}>
          You haven't set up your routine yet. Please select your courses to get started.
        </p>
        <Link
          to="/routine-setup"
          style={{
  ...
  // Handle empty picks array
  if (my.picks && my.picks.length === 0) {
    return (
      <div style={{ padding: 20, textAlign: "center" }}>
        <h3>Routine is Empty</h3>
        <p>Please add some courses to your routine.</p>
        <Link to="/routine-setup">Go to Setup</Link>
      </div>
    );
  }
