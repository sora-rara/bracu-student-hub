...
// Pre-save hook to calculate grade points and semester GPA
...
courseGradeSchema.statics.calculateSequentialCGPA = async function (studentId) {
    ...
    semesters.forEach((sem, index) => {
        if (index === 0) cumulativeGPA = sem.semesterGPA;
        else cumulativeGPA = ((cumulativeGPA * index) + sem.semesterGPA) / (index + 1);
    });
