...
const requireAuth = async (req, res, next) => {
    ...
    // Check session.user first, fallback to session.userId
...
const requireAdmin = async (req, res, next) => {
    ...
    try {
        ...
        if (userData.role !== 'admin') {
            console.log('❌ User is not admin:', { id: userData._id, email: userData.email, role: userData.role });
            return res.status(403).json({ success: false, message: "Admin access required" });
        }

        console.log('✅ Admin access granted:', { id: userData._id, email: userData.email, role: userData.role });
...
const checkReviewOwnership = async (req, res, next) => {
    try {
        ...
        if (!isOwner && !isAdmin) return res.status(403).json({ success: false, message: 'Not authorized to modify this review' });
...
// ====================
// COURSE CONTENT OWNERSHIP CHECK
// ====================
const checkContentOwnership = async (req, res, next) => {
    try {
        ...
        if (!isOwner && !isAdmin) return res.status(403).json({ success: false, message: 'Not authorized to modify this content' });
